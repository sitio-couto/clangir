...
Type X86_64ABIInfo::GetSSETypeAtOffset(Type IRType, unsigned IROffset,
                                       Type SourceTy,
                                       unsigned SourceOffset) const {
  const CIRDataLayout &TD = getDataLayout();
  unsigned SourceSize =
      (unsigned)getContext().getTypeSize(SourceTy) / 8 - SourceOffset;
  Type T0 = getFPTypeAtOffset(IRType, IROffset, TD);
  if (!T0 || T0.isa<Float64Type>())
  ...
  if (T1 == nullptr) {
    ...
    if (T0.isa<Float16Type>() && SourceSize > 4)
      llvm_unreachable("NYI");
...
Type X86_64ABIInfo::GetINTEGERTypeAtOffset(Type DestTy, unsigned IROffset,
  ...
  // CIR, mostly because coerced types should not be sign/zero extended. On
  // the other hand, this might not make a difference in practice. For now, we
  // just preserve the sign as is to avoid unecessary bitcasts.
  bool isSigned = false;
...
void X86_64ABIInfo::classify(Type Ty, uint64_t OffsetBase, Class &Lo, Class &Hi,
  ...
  // FIXME(cir): The condition below should check if the type is a buitin
